...
export function HomePage() {
  ...
  // Reset the auto-generate guard when the current story is insufficient,
  // so editing memories with real content triggers a new attempt
  if (isInsufficientStory && !generateStoryMutation.isPending && successMemories.length >= MIN_MEMORIES_FOR_STORY) {
    hasAutoGenerated.current = false;
  }
  const isFirstStoryNotGenerated = (!storyQuery.data?.story || isInsufficientStory) && !storyQuery.isLoading && !storyQuery.isError;
  useEffect(() => {
    if (
                  ...
                  <div className="date-type-selector">
                    ...
                    {dateType === 'Age' && (
                      <div className="date-value-input">
                        ...
                        {dateValues.Age && !getAgeError() && (
                          <div className="date-value-hint">
                            ℹ️ Aproximadamente {currentYear - parseInt(dateValues.Age, 10)}.
                      ...
                      onClick={() => {
                        ...
                        createMemoryMutation.mutate(payload, {
                          onSuccess: () => {
                            ...
                            setTitle(''); setContent(''); setDateType(null); setDateValues(EMPTY_DATE_VALUES);
                          },
                          onError: (error) => {
                            const msg = error && typeof error === 'object' && 'message' in error
                              ? (error as { message: string }).message
