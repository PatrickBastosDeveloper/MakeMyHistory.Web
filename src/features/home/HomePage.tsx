...
export function HomePage() {
  ...
  function getFullDateError(): string | null {
    if (!dateValues.FullDate || dateType !== 'FullDate') return null;
  ...
  function getYearError(): string | null {
    if (!dateValues.YearOnly) return null;
    if (!/^\d+$/.test(dateValues.YearOnly)) return 'Use apenas números.';
    const y = Number(dateValues.YearOnly);
    if (y < 1900 || y > currentYear) return `O ano deve estar entre 1900 e ${currentYear}.`;
    if (y > currentYear) return 'O ano informado é futuro.';
    return null;
  }

  function getAgeError(): string | null {
    if (!dateValues.Age) return null;
    if (!/^\d+$/.test(dateValues.Age)) return 'Use apenas números.';
    const a = Number(dateValues.Age);
      ...
      generateStoryMutation.mutate(
        { userId },
        {
          onSuccess: () => {
            ...
            // invalidateQueries no hook already triggers refetch
          },
          onError: () => {
            // Only reset guard for network errors (no response), not server errors
            // Server errors (400/500) are permanent - don't retry
            showToast({
  ...
  }, [successMemories.length, isFirstStoryNotGenerated, userId, generateStoryMutation, showToast]);
  ...
  const canSubmit = Boolean(content.trim()) && !createMemoryMutation.isPending && !hasDateError && (dateType === null || currentDateValue !== '');
                          ...
                          <div className="date-value-input">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                let formatted = raw;
                                if (raw.length === 2 && !raw.includes('/') && dateValues.FullDate.length < 2) formatted = raw + '/';
                                else if (raw.length === 5 && raw[2] === '/' && !raw.slice(3).includes('/') && dateValues.FullDate.length < 5) formatted = raw + '/';
                      ...
                      onClick={() => {
                        ...
                        if (dateType && dateType === 'FullDate' && dateValues.FullDate) {
                          ...
                          if (parts.length === 3) {
                            ...
                            payload.dateType = 'FullDate';
                            payload.eventDate = dateValues.FullDate;
                          }
                        ...
                        createMemoryMutation.mutate(payload, {
                          onSuccess: () => {
                            ...
                            setTitle(''); setContent(''); setDateType(null); setDateValues(EMPTY_DATE_VALUES); setNoDate(false);
                          },
                          onError: (error) => {
                            const msg = error && typeof error === 'object' && 'message' in error
                              ? (error as { message: string }).message
              ...
              <section className="panel-card">
                ...
                <div className="story-two-cards">
                  ...
                  {generateStoryMutation.isPending ? (
                    ...
                    // Check if story is the "insufficient information" message or marker
                    (storyQuery.data.story.content === 'INSUFFICIENT_INFORMATION' ||
                     storyQuery.data.story.content === 'Ainda não há informações suficientes para criar uma história significativa. Adicione mais detalhes às suas memórias.') ? (
                      <div className="story-placeholder-card">
